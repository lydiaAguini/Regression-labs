...
class LWR(Gaussians):
    ...
    def plot(self, x_data, y_data, ax=None):
        ...
        if ax is None:
            ...
            ax.plot(x_data, y_data, 'o', markersize=3, color='lightgreen')
            ax.plot(xs, z, lw=2, color='red')
            for i in range(self.nb_features):
                ww = (1.0 - 0.0) / self.nb_features / 2.
                xstmp = np.linspace(self.centers[i] - ww, self.centers[i] + ww, 100)
                ...
                for j in xstmp:
                    z2.append(self.feature(j, i))
                ax.plot(xstmp, z2, lw=2, color='blue', ls='-')
